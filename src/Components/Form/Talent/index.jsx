...
const TalentComponent = () => {
  ...
  const submitHandler = async (e) => {
    ...
    const response = await axios.post("http://wren.in:3200/api/sign-up/talent", fanData);
    if (response.status !== 201) {
      alert("Somethings wants to wrong!!");
      return;
    }
    alert("Congratulations! Talent data created");
    resetData();
  }
  return (
    <Form className="form" onSubmit={submitHandler}>
      <Form.Label className="form-heading">Create Your Talent Account</Form.Label>
      <FormComponent
        firstName={firstName}
